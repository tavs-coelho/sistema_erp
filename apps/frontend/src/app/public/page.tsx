  ...
  return (
    <main className="module-page">
      <div>
        ...
        <p className="muted">
          Consulta pública irrestrita de despesas, contratos, licitações, convênios e arrecadação.
      ...
      {tab === "inicio" && (
        <section className="page-section">
          <p>Indicadores consolidados do município — atualizados em tempo real a partir dos sistemas internos.</p>
          {stats ? (
      ...
      {tab === "empenhos" && (
        <section className="page-section">
          <div className="toolbar">
            <input className="flex-1" placeholder="Buscar descrição ou número" value={empSearch}
              onChange={(e) => { setEmpSearch(e.target.value); setEmpPage(1); }} />
            <Button onClick={() => { setEmpPage(1); loadEmpenhos(); }}>Buscar</Button>
            <a className="btn btn-export" href={csvUrl("/public/commitments", { search: empSearch })} target="_blank" rel="noreferrer">Exportar CSV</a>
          </div>
          <table>
            <thead><tr><th>Número</th><th>Descrição</th><th>Valor</th><th>Status</th></tr></thead>
            <tbody>
              {(empenhos?.items || []).length > 0 ? empenhos!.items.map((r) => (
                <tr key={r.id}><td>{r.number}</td><td>{r.description}</td><td className="text-right">{fmt(r.amount)}</td><td><span className={`chip ${r.status}`}>{r.status}</span></td></tr>
      ...
      {tab === "contratos" && (
        <section className="page-section">
          <div className="toolbar">
            <input className="flex-1" placeholder="Número do contrato" value={ctSearch} onChange={(e) => setCtSearch(e.target.value)} />
            ...
            <Button onClick={() => { setCtPage(1); loadContratos(); }}>Filtrar</Button>
            <a className="btn btn-export" href={csvUrl("/public/contracts", { search: ctSearch, status: ctStatus })} target="_blank" rel="noreferrer">Exportar CSV</a>
          </div>
          <table>
            ...
            <tbody>
              {(contratos?.items || []).length > 0 ? contratos!.items.map((r) => (
                <tr key={r.id}><td>{r.number}</td><td>{r.start_date}</td><td>{r.end_date}</td><td className="text-right">{fmt(r.amount)}</td><td><span className={`chip ${r.status}`}>{r.status}</span></td></tr>
      ...
      {tab === "licitacoes" && (
        <section className="page-section">
          <div className="toolbar">
            <input className="flex-1" placeholder="Número ou objeto" value={lcSearch} onChange={(e) => setLcSearch(e.target.value)} />
            ...
            <Button onClick={() => { setLcPage(1); loadLicitacoes(); }}>Filtrar</Button>
            <a className="btn btn-export" href={csvUrl("/public/licitacoes", { search: lcSearch, status: lcStatus })} target="_blank" rel="noreferrer">Exportar CSV</a>
          </div>
          <table>
      ...
      {tab === "convenios" && (
        <section className="page-section">
          <div className="toolbar">
            <input className="flex-1" placeholder="Número, objeto ou concedente" value={cvSearch} onChange={(e) => setCvSearch(e.target.value)} />
            <select value={cvTipo} onChange={(e) => setCvTipo(e.target.value)}>
              <option value="">Todos os tipos</option>
              <option value="recebimento">Recebimento</option>
              <option value="repasse">Repasse</option>
            </select>
            <Button onClick={() => { setCvPage(1); loadConvenios(); }}>Filtrar</Button>
            <a className="btn btn-export" href={csvUrl("/public/convenios", { search: cvSearch, tipo: cvTipo })} target="_blank" rel="noreferrer">Exportar CSV</a>
          </div>
          <table>
            <thead><tr><th>Número</th><th>Objeto</th><th>Concedente</th><th>Tipo</th><th className="text-right">Valor total</th><th>Assinatura</th><th>Status</th></tr></thead>
            <tbody>
              {(convenios?.items || []).length > 0 ? convenios!.items.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.concedente}</td>
                  <td>{r.tipo}</td>
                  <td className="text-right">{fmt(r.valor_total)}</td>
      ...
      {tab === "arrecadacao" && (
        <section className="page-section">
          <div className="toolbar">
            ...
            <Button onClick={() => { setArPage(1); loadArrecadacao(); }}>Filtrar</Button>
            <a className="btn btn-export" href={csvUrl("/public/arrecadacao", { tributo: arTributo })} target="_blank" rel="noreferrer">Exportar CSV</a>
          </div>
          <table>
            <thead><tr><th>Tributo</th><th>Competência</th><th>Exercício</th><th className="text-right">Valor</th><th>Data pagamento</th></tr></thead>
            <tbody>
      ...
      {tab === "divida" && (
        <section className="page-section">
          ...
          <div className="toolbar">
            ...
            <Button onClick={() => { setDvPage(1); loadDividas(); }}>Filtrar</Button>
            <a className="btn btn-export" href={csvUrl("/public/divida-ativa", { tributo: dvTributo })} target="_blank" rel="noreferrer">Exportar CSV</a>
          </div>
          <table>
            <thead><tr><th>Inscrição</th><th>Tributo</th><th>Exercício</th><th className="text-right">Valor original</th><th className="text-right">Valor atualizado</th><th>Data inscrição</th><th>Status</th></tr></thead>
            <tbody>
              {(dividas?.items || []).length > 0 ? dividas!.items.map((r, i) => (
                <tr key={i}>
                  ...
                  <td>{r.tributo}</td>
                  <td>{r.exercicio}</td>
                  <td className="text-right">{fmt(r.valor_original)}</td>
                  <td className="text-right">{fmt(r.valor_atualizado)}</td>
